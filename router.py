...
shouldDrop = [3, 5, 9, 13, 23, 24, 32, 34, 36, 40, 42, 45, 49, 50, 53, 55, 58, 75, 77, 79, 80, 81, 83, 85, 86, 87, 88, 90, 93, 95, 96, 98, 105, 106, 122, 123, 124, 125, 126, 130, 140, 152, 167, 170, 177, 186, 192, 193, 196, 199, 200, 205, 206, 207, 213, 218, 223, 227, 230, 243, 247, 251, 258, 265, 270, 289, 292, 296, 298, 300, 303, 312, 313, 314, 315, 316, 317, 319, 321, 325, 327, 330, 331, 332, 333, 334, 335, 337, 345, 347, 368, 379, 387, 388, 393, 395, 397, 398, 399, 402, 405, 407, 408, 411, 419, 422, 424, 431, 436, 437, 453, 464, 465, 466, 469, 472, 475, 484, 500, 504, 509, 516, 521, 522, 526, 527, 538, 547, 548, 552, 558, 563, 572, 573, 575, 577, 596, 618, 619, 625, 635, 636, 645, 657, 658]
shouldBuy = [66, 102, 112, 146, 153, 165, 234, 241, 275, 367, 378, 381, 382, 383, 384, 385, 389, 394, 427, 433, 435, 442, 480, 494, 543, 566, 580, 581, 586, 588, 592, 607, 616, 665, 666, 673]
mustFuse = [ 22, 37, 64, 69, 72, 217, 318, 348, 545, 667, 669, 689, 696]
shouldFuse = [2, 10, 11, 15, 31, 33, 39, 41, 73, 84, 97, 99, 100, 117, 133, 138, 154, 157, 168, 215, 244, 272, 294, 301, 302, 304, 305, 306, 307, 308, 309, 310, 311, 320, 322, 323, 324, 326, 328, 329, 336, 340, 341, 342, 346, 375, 376, 377, 390, 392, 401, 404, 409, 412, 413, 423, 425, 426, 430, 434, 438, 440, 443, 448, 449, 450, 456, 458, 459, 460, 462, 467, 471, 473, 479, 483, 487, 495, 502, 508, 511, 518, 519, 520, 529, 531, 533, 546, 551, 564, 567, 570, 571, 582, 587, 593, 594, 595, 610, 613, 617, 620, 626, 627, 633, 638, 639, 641, 642, 643, 647, 650, 651, 654, 656, 659, 660, 661, 662, 663, 664, 668, 670, 671, 672, 674, 675, 676, 677, 678, 679, 680, 681, 682, 683, 684, 685, 686, 687, 688, 690, 691, 692, 693, 694, 695, 697, 698, 699, 700]
rituals = [ 356, 357, 360, 364, 365, 374, 380, 701, 702, 703, 704, 705, 706, 708, 710, 715, 716, 718, 719, 720]
unoptainable = [ 7, 17, 18, 28, 51, 52, 56, 57, 60, 62, 63, 67, 235, 252, 284, 288, 299, 369, 428, 429, 499, 541, 554, 555, 562, 603, 628, 640, 709, 711, 717, 721, 722]
dropable = [i for i in range(1,723) if i not in unoptainable ]
...
def printXs():
    outFile = open("outX.dat", "w")
    for i in range(1,len(cardsInLib)):
        dist = cardsInLib[i]-cardsInLib[i-1]-1
        #print("%sx"%(lines[i-1].strip()))
        outFile.write("x\n")
        for _ in range(dist):
            outFile.write("\n")
            #print(int(lines[i-1])+1+j)
...
def fusionChecker(listCards):
    if len(listCards) > 0:
        ...
        if len(idStr) > 0:
            ...
            mycursor.execute(sqlCMD)
            fusList = mycursor.fetchall()
            for result,card1,card2 in fusList:
                if card1 in cardsInLib and card2 in cardsInLib:
                    print("%s <<< %s + %s"%(cardNames[result-1],cardNames[card1-1],cardNames[card2-1]))
...
def starsLeft():
    ...
    sqlCMD += '('+ idStr[:-1] + ');'
    mycursor.execute(sqlCMD)
    print(f'{cleanFetch()[0]:,}'+" stars needed.\r\n")
